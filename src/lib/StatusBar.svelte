<script lang="ts">
  ...
  const resting = $derived(statusSummary(appState.project, appState.selection, appState.playRange));
</script>
...
<!-- h-7 rather than an arbitrary 22px: it matches the ruler / add-track strip, so the app has one
     ...
     nothing else uses. -->
<div class="flex h-7 shrink-0 items-center gap-2 border-t border-line bg-panel px-3 text-[11px]">
