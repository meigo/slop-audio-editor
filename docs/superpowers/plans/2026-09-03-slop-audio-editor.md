...
- Produces:
  ...
  - `beginGesture()` / `amend(fn)` / `endGesture()` — one history entry per drag
...
- [ ] **Step 5: Research the encoder question, then write `src/export/encode.ts`**

Before writing the file, answer this and record the answer as a comment at the top of `encode.ts`:

> Which audio-only output formats can mediabunny actually produce in this browser, and is MP3 among them?

Check, in order: mediabunny's own output-format API and its `getSupportedAudioCodecs()` (or the current equivalent) for the formats it exposes; then `AudioEncoder.isConfigSupported()` for `mp4a.40.2` (AAC), `opus`, and `mp3`. Report the finding in the task's review. **If MP3 turns out to be unavailable, leave it out** — WAV, M4A and WebM cover the use cases, and adding lamejs is a separate decision for the user, not a silent one.

```ts
import { encodeWav } from "./wav";
import type { ExportFormat } from "./formats";

// RESEARCH RESULT (fill in when Step 5's question is answered):
// mediabunny audio-only outputs available here: …
// AudioEncoder support: aac=… opus=… mp3=…
...
/** Only offer what this browser can actually produce — a format that is offered and then fails
 *  is worse than one that was never listed. WAV is always present. */
export async function availableFormats(): Promise<ExportFormat[]> {
  const formats: ExportFormat[] = ["wav16", "wav32"];
  if (typeof AudioEncoder === "undefined") return formats;
  ...
  if (await probe("mp4a.40.2")) formats.push("m4a");
  if (await probe("opus")) formats.push("webm");
  return formats;
}
...
export async function encodeBuffer(buffer: AudioBuffer, format: ExportFormat): Promise<Blob> {
  ...
  // mediabunny path — written against whatever Step 5's research established as its current API:
  // build an Output with the matching audio-only format, add an AudioBufferSource for `buffer`,
  // start(), finalize(), and return the resulting Blob.
  throw new Error(`Encoder for ${format} not implemented`);
}
```

**Replace that final `throw` with the real mediabunny implementation in this same task** — the throw exists only so the file type-checks while you are researching. A task that ends with it still present is not done.

- [ ] **Step 6: Write `src/lib/ExportDialog.svelte`**

A modal with: a format `<select>` populated from `availableFormats()`, the computed range (whole project or "selection: mm:ss–mm:ss"), a progress line while rendering, and an Export button that runs `mixdown` → `encodeBuffer` → triggers a download via an object URL. Disable the button while rendering, and surface any thrown error as text in the dialog rather than only in the console.

- [ ] **Step 7: Add an Export button to `Toolbar.svelte`** that opens the dialog.

- [ ] **Step 8: Manual verification**

- Export a two-track project as WAV; open it in another player and confirm the length, the mix balance, and that fades are audible.
- **Solo a track, then export: the exported file must contain BOTH tracks.** This is the single most important check in the plan — it is the invariant the whole `planSchedule` design exists to guarantee.
- **Mute a track, then export: the muted track must be absent.**
- Select a time range and export: the file covers exactly that range.
- Export the same project as MP3 and as M4A and confirm each sounds like the WAV.
- Confirm the format list contains only formats that actually succeed.
